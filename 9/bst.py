...
# Driver
def main():
    # Initialize roots and tmp nodes
    SHroot = None
    GoTroot = None
    HProot = None
    student = None
    ...
    ### MANIPULATION OF SUPERHERO LIST
    # Print unsorted list of student data
    print("---Unsorted Superhero List---")
    print(SHList)

    # Build Tree from sList
    for i in range (0, len(SHList)):
        nNode = Node(i, SHList[i][0])
        SHroot = insertNode(SHroot, nNode)
        # print("New Node: ", nNode.val)
    ...
    # Search for students
    print("\n---Search---")
    student = find(SHroot, 33744)
    print("Found student", student.val[1], ":", SHList[student.val[0]])

    student = find(SHroot, 53666)
    print("Found student", student.val[1], ":", SHList[student.val[0]])

    # Delete records
    print("\n---Delete--")
    print("Remove 'Matt Murdock'")
    SHroot = delNode(SHroot, 52267)

    print("---In Order Traversal After Delete 'Matt Murdock'---")
    trav(SHroot, SHList)

    print("\nRemove 'Carol Danvers'")
    SHroot = delNode(SHroot, 62755)

    print("---In Order Traversal After Delete 'Carol Danvers'---")
    trav(SHroot, SHList)
    ...
    SHroot = insertNode(SHroot, tmpNode)

    # Print unsorted list after adding new record
    print("\n---Unsorted Superhero List After Addition---")
    print(SHList)

    # Print BST after adding new record
    print("\n---In Order Traversal After Adding Record---")
    trav(SHroot, SHList)

    ### MANIPULATION OF GOT LIST
    # Print unsorted list of student data
    print("\n\n\n---Unsorted Westeros List---")
    print(GoTList)

    # Build Tree from sList
    for i in range (0, len(GoTList)):
        nNode = Node(i, GoTList[i][0])
        GoTroot = insertNode(GoTroot, nNode)

    # Print in order traversal of tree
    print("\n---In Order Traversal---")
    trav(GoTroot, GoTList)

    # Search for students
    print("\n---Search---")
    student = find(GoTroot, 854374)
    print("Found student", student.val[1], ":", GoTList[student.val[0]])

    student = find(GoTroot, 729463)
    print("Found student", student.val[1], ":", GoTList[student.val[0]])

    # Delete records
    print("\n---Delete--")
    print("Remove 'Olenna Tyrell'")
    GoTroot = delNode(GoTroot, 121212)

    print("---In Order Traversal After Delete 'Olenna Tyrell'---")
    trav(GoTroot, GoTList)

    print("\nRemove 'Petyr Baelish'")
    GoTroot = delNode(GoTroot, 228301)

    print("---In Order Traversal After Delete 'Petyr Baelish'---")
    trav(GoTroot, GoTList)

    # Insert new record to list
    GoTList.append((339701, "Daenerys Targaryen", [95, 81, 72, 100, 87, 93, 97, 84, 91, 93]))

    # Insert new record to BST
    newI = (len(GoTList) - 1)
    tmpNode = Node(newI, GoTList[newI][0])
    GoTroot = insertNode(GoTroot, tmpNode)

    # Print unsorted list after adding new record
    print("\n---Unsorted Westeros List After Addition---")
    print(GoTList)
    ...
    ### MANIPULATION OF HP LIST
    # Print unsorted list of student data
    print("\n\n\n---Unsorted Non-Muggles List---")
    print(HPList)

    # Build Tree from sList
    for i in range (0, len(HPList)):
        nNode = Node(i, HPList[i][0])
        HProot = insertNode(HProot, nNode)
    ...
    # Search for students
    print("\n---Search---")
    student = find(HProot, 1526)
    print("Found student", student.val[1], ":", HPList[student.val[0]])

    student = find(HProot, 7640)
    print("Found student", student.val[1], ":", HPList[student.val[0]])

    # Delete records
    print("\n---Delete--")
    print("Remove 'Dolores Umbridge'")
    HProot = delNode(HProot, 4731)

    print("---In Order Traversal After Delete 'Dolores Umbridge'---")
    trav(HProot, HPList)

    print("\nRemove 'Tom M. Riddle'")
    HProot = delNode(HProot, 5432)

    print("---In Order Traversal After Delete 'Tom M. Riddle'---")
    trav(HProot, HPList)
    ...
    HProot = insertNode(HProot, tmpNode)

    # Print unsorted list after adding new record
    print("\n---Unsorted Non-Muggle List After Addition---")
    print(HPList)
